...
const progressColor =
  points >= 200 ? "linear-gradient(90deg,#FFD700,#FFF4B0,#FFD700)" :
  points >= 150 ? "#60A5FA" :
  points >= 100 ? "#D4AF37" :
  points >= 50 ? "#C0C0C0" :
  "#CD7F32";
...
const level =
  ...
  return (
    <div className="min-h-screen bg-base text-white" data-testid="landing-page">
      {/* Hero */}
      <section className="relative overflow-hidden min-h-[48vh] lg:min-h-[78vh] flex items-start pt-5">
        <div
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-10 w-full">
          <div className="max-w-4xl mx-auto text-center">
            ...
            {/* Crown badge */}
            <div className="inline-flex items-center justify-center w-14 h-14 lg:w-20 lg:h-20 rounded-full bg-gold shadow-[0_0_50px_rgba(255,215,0,0.5)] mb-2 animate-fade-in-up">
              <Crown className="w-8 h-8 lg:w-11 lg:h-11 text-black" strokeWidth={2.3} />
            </div>
            ...
            {user ? (
<div className="mx-auto max-w-md rounded-3xl border border-gold/20 bg-black/40 backdrop-blur-xl p-6 mb-1 shadow-[0_0_40px_rgba(255,215,0,.15)] animate-fade-in-up user-card">
<div className="flex items-center justify-between">
<div>
<p className="text-zinc-400 text-sm">👋 أهلاً بعودتك</p>
<h2 className="text-3xl font-black text-gold mt-1">{user.name}</h2>
<p className="text-zinc-300 mt-2">{level}</p>
</div>
<div className="rounded-3xl border-[3px] border-gold p-1.5 shadow-[0_0_35px_rgba(212,175,55,.55)] transition-all duration-300 hover:scale-105">
  {user?.avatar ? (
    <Avatar
      src={user.avatar}
      name={user.name}
    ...
    <div className="w-14 h-14 rounded-xl bg-gold flex items-center justify-center text-black text-3xl">
      👑
    </div>
  )}
</div>
...
{stats?.rank && (
  <div className="mb-3 flex items-center justify-between rounded-xl border border-gold/20 bg-gold/5 px-3 py-2">
...
<div
  className="h-full bg-gold rounded-full progress-shine"
  style={{
    width: `${Math.min(
      ((user?.total_points ?? 0) >= 200
        ? 100
        : (((user?.total_points ?? 0) % 50) / 50) * 100),
      100
    )}%`,
  }}
></div>
...
<div className="flex items-center justify-between mt-2 text-[11px]">
  ...
  <span className="text-gold font-semibold">
    {(user?.total_points ?? 0) >= 200
      ? "👑 وصلت إلى أعلى مستوى"
      : `🎯 بقي ${
          50 - ((user?.total_points ?? 0) % 50)
        } نقطة للوصول إلى ${
          (user?.total_points ?? 0) >= 150 ? "👑 ملك التوقعات" :
          (user?.total_points ?? 0) >= 100 ? "💎 بلاتيني" :
          (user?.total_points ?? 0) >= 50 ? "🥇 ذهبي" :
          "🥈 فضي"
        }`}
  </span>
              ...
              <h1 className="font-display text-3xl sm:text-5xl lg:text-8xl font-black leading-[1.05] mb-2 animate-fade-in-up">
            ...
            <p className="text-sm sm:text-xl text-zinc-300 leading-relaxed mb-1 max-w-2xl mx-auto animate-fade-in-up">
              {user
            ...
            {/* CTAs */}
            <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto animate-fade-in-up">
              {user ? (
                <>
                  <Link
                    to="/matches"
                    data-testid="cta-go-to-matches"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gold text-black font-bold text-sm cta-premium hover:bg-yellow-400 active:scale-95 transition-all shadow-[0_8px_40px_rgba(255,215,0,0.35)]"
                  >
                  ...
                  <Link
                    to="/leaderboard"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-gold/30 bg-black/30 backdrop-blur text-white font-bold hover:bg-gold/10 transition-colors"
                  >
                ...
                <>
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-white/20 bg-black/30 backdrop-blur text-white font-medium hover:bg-white/5 transition-colors"
                  >
                    {t("landing_cta_login")}
                  </Link>
                </>
