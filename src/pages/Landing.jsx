import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import api from "../lib/api";
import AdSlider from "../components/AdSlider";
import { useAuth } from "../context/AuthContext";
import { useContent } from "../context/ContentContext";
import { Crown, Trophy, Target, ChevronLeft, Sparkles, TrendingUp, CalendarDays, Flame } from "lucide-react";
import PushEnableBanner from "../components/PushEnableBanner";
import HeroSection from "../components/landing/HeroSection";
import HomeDashboard from "@/components/android/HomeDashboard";
import UserCard from "../components/landing/UserCard";
import CountdownCard from "../components/landing/CountdownCard";
import QuickActions from "../components/landing/QuickActions";
import FeatureCard from "../components/landing/FeatureCard";
import Avatar from "../components/Avatar";

const HERO_BG =
  "https://static.prod-images.emergentagent.com/jobs/36ca7cb1-56bf-4cbb-a9df-d07fc64af674/images/cf694e18d81a1e3b6118c5df05310e8b849aa4f259b4293ec8caeb6d2c40b94f.png";



export default function Landing() {
  const { user } = useAuth();
  const { t } = useContent();
const [stats, setStats] = useState(null);
  const [marquee, setMarquee] = useState(
    "🏆 الرعاة الرسميون لجائزة ملك التوقعات"
  );

  const points = Number(user?.total_points ?? 0);

    
const progressColor =
  points >= 200 ? "linear-gradient(90deg,#FFD700,#FFF4B0,#FFD700)" :
  points >= 150 ? "#60A5FA" :
  points >= 100 ? "#D4AF37" :
  points >= 50 ? "#C0C0C0" :
  "#CD7F32";

const level =
      points >= 200 ? "👑 ملك التوقعات" :
      points >= 150 ? "💎 بلاتيني" :
      points >= 100 ? "🥇 ذهبي" :
      points >= 50 ? "🥈 فضي" :
      "🥉 برونزي";

  useEffect(() => {
    api.get("/marquee")
      .then((r) => setMarquee(r.data?.text || ""))
      .catch((e) => console.error("Failed to load marquee", e));
  }, []);

  useEffect(() => {
    let active = true;

    (async () => {
      try {
        const { data } = await api.get("/stats/me");
        if (active) setStats(data);
      } catch (e) {
        console.error("Failed to load stats", e);
      }
    })();

    return () => {
      active = false;
    };
  }, []);


  return (
    <div className="landing-premium min-h-screen bg-base text-white" data-testid="landing-page">
      {/* Hero */}
      <section className="landing-hero relative overflow-hidden md:min-h-[78vh] flex items-start pt-3 md:pt-5">
        <div
          className="absolute inset-0 bg-cover bg-center opacity-50"
          style={{ backgroundImage: `url(${HERO_BG})` }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0A0A0A]/40 via-[#0A0A0A]/70 to-[#0A0A0A]" />

        {/* Decorative glow */}
        <div className="absolute -top-32 right-1/2 translate-x-1/2 w-[40rem] h-[40rem] rounded-full bg-gold/10 blur-[120px] pointer-events-none" />

        <div className="landing-hero-content relative max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-2 md:pt-4 pb-4 md:pb-10 w-full">
          <div className="max-w-4xl mx-auto text-center">
            <PushEnableBanner />
            <div className="landing-sponsor-slot">
              <AdSlider />
            </div>

            {marquee && (
              <div className="home-marquee-bar landing-marquee">
                <div className="home-marquee-track">
                  {marquee}
                </div>
              </div>
            )}

            {/* Crown badge */}
            <div className="landing-crown inline-flex items-center justify-center w-12 h-12 md:w-20 md:h-20 rounded-full bg-gold shadow-[0_0_35px_rgba(255,215,0,0.35)] mb-1 md:mb-2 animate-fade-in-up">
              <Crown className="w-7 h-7 md:w-11 md:h-11 text-black" strokeWidth={2.3} />
            </div>

            <div className="landing-badge inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-gold/10 border border-gold/30 mb-2 animate-fade-in-up">
              <Sparkles className="w-4 h-4 text-gold" />
              <span className="text-xs font-bold text-gold uppercase tracking-[0.25em]">
                {t("landing_badge")}
              </span>
              <Sparkles className="w-4 h-4 text-gold" />
            </div>

            {user ? (
<div className="mx-auto w-full max-w-[360px] md:max-w-md rounded-[22px] md:rounded-3xl border border-gold/20 bg-black/55 backdrop-blur-xl p-4 md:p-6 mb-1 shadow-[0_0_30px_rgba(255,215,0,.10)] animate-fade-in-up user-card landing-user-card">
<div className="flex items-center justify-between">
<div>
<p className="text-zinc-400 text-[12px] md:text-sm">👋 أهلاً بعودتك</p>
<h2 className="text-[25px] md:text-3xl leading-tight font-black text-gold mt-1">{user.name}</h2>
<p className="text-zinc-300 text-[14px] md:text-base mt-1 md:mt-2">{level}</p>
</div>
<div className="rounded-[20px] md:rounded-3xl border-[3px] border-gold p-1 shadow-[0_0_25px_rgba(212,175,55,.35)] transition-all duration-300 hover:scale-105">
  {user?.avatar ? (
    <Avatar
      src={user.avatar}
      name={user.name}
      size={58}
    />
  ) : (
    <div className="w-14 h-14 rounded-xl bg-gold flex items-center justify-center text-black text-3xl">
      👑
    </div>
  )}
</div>
</div>
<div className="mt-2 md:mt-3">

{stats?.rank && (
  <div className="mb-2 md:mb-3 flex items-center justify-between rounded-xl border border-gold/20 bg-gold/5 px-3 py-1.5 md:py-2">

    <span className="text-zinc-300 text-sm">
      🏆 ترتيبك
    </span>

    <span className="font-black text-gold">
      #{stats.rank}
    </span>

  </div>
)}
<div className="flex items-center justify-between mb-2">

<span className="text-zinc-300 font-medium">
النقاط
</span>

<span className="text-gold font-black text-xl md:text-2xl tracking-wide">
{user.total_points}
</span>

</div>
<div className="h-2.5 md:h-3 rounded-full bg-zinc-800 overflow-hidden">
<div
  className="h-full bg-gold rounded-full progress-shine"
  style={{
    width: `${Math.min(
      ((user?.total_points ?? 0) >= 200
        ? 100
        : (((user?.total_points ?? 0) % 50) / 50) * 100),
      100
    )}%`,
  }}
></div>
</div>

<div className="flex items-center justify-between mt-2 text-[11px]">

  

  <span className="text-gold font-semibold">
    {(user?.total_points ?? 0) >= 200
      ? "👑 وصلت إلى أعلى مستوى"
      : `🎯 بقي ${
          50 - ((user?.total_points ?? 0) % 50)
        } نقطة للوصول إلى ${
          (user?.total_points ?? 0) >= 150 ? "👑 ملك التوقعات" :
          (user?.total_points ?? 0) >= 100 ? "💎 بلاتيني" :
          (user?.total_points ?? 0) >= 50 ? "🥇 ذهبي" :
          "🥈 فضي"
        }`}
  </span>

</div>

</div>
</div>
) : (
              <h1 className="font-display text-3xl sm:text-5xl lg:text-8xl font-black leading-[1.05] mb-2 animate-fade-in-up">
                <span className="text-white">{t("landing_hero_line1")}</span>
                <br />
                <span className="text-gold drop-shadow-[0_0_30px_rgba(255,215,0,0.4)]">
                  {t("landing_hero_line2_strong")}
                </span>
                <br />
                <span className="italic text-white/95">{t("landing_hero_line3_italic")}</span>
              </h1>
            )}

            <p className="landing-mission text-[13px] sm:text-xl text-zinc-300 leading-5 md:leading-relaxed mb-1 max-w-2xl mx-auto animate-fade-in-up px-2">
              {user
                ? "كأس العالم 2026 على الأبواب — جاهز لتحطيم رقمك القياسي؟"
                : t("landing_hero_desc")}
            </p>
{/* CTAs */}
            <div className="landing-cta-grid grid grid-cols-2 gap-2 md:gap-3 max-w-[360px] md:max-w-sm mx-auto animate-fade-in-up">
              {user ? (
                <>
                  <Link
                    to="/matches"
                    data-testid="cta-go-to-matches"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gold text-black font-bold text-sm cta-premium hover:bg-yellow-400 active:scale-95 transition-all shadow-[0_8px_40px_rgba(255,215,0,0.35)]"
                  >
                    <Flame className="w-4 h-4" />
                    {t("landing_cta_authenticated")}
                    <ChevronLeft className="w-4 h-4" />
                  </Link>
                  <Link
                    to="/leaderboard"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-gold/30 bg-black/30 backdrop-blur text-white font-bold hover:bg-gold/10 transition-colors"
                  >
                    <Trophy className="w-4 h-4 text-gold" />
                    لوحة المتصدرين
                  </Link>
                </>
              ) : (
                <>
                  <Link
                    to="/register"
                    data-testid="cta-register"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gold text-black font-bold text-sm cta-premium hover:bg-yellow-400 active:scale-95 transition-all shadow-[0_8px_40px_rgba(255,215,0,0.35)]"
                  >
                    {t("landing_cta_register")}
                    <ChevronLeft className="w-4 h-4" />
                  </Link>
                  <Link
                    to="/login"
                    data-testid="cta-login"
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-white/20 bg-black/30 backdrop-blur text-white font-medium hover:bg-white/5 transition-colors"
                  >
                    {t("landing_cta_login")}
                  </Link>
                </>
              )}
            </div>

            {!user && (
              <p className="text-xs text-zinc-500 mt-3 animate-fade-in-up">
                مجاني تماماً • انضم لآلاف المتسابقين • نافس على لقب الملك
              </p>
            )}
          </div>
        </div>
      </section>

      <HomeDashboard user={user} />

      {/* Features */}}
      <section className="hidden md:block relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <FeatureCard icon={Target} title={t("feature_1_title")} desc={t("feature_1_desc")} />
          <FeatureCard icon={Trophy} title={t("feature_2_title")} desc={t("feature_2_desc")} />
          <FeatureCard icon={CalendarDays} title={t("feature_3_title")} desc={t("feature_3_desc")} />
        </div>

        {/* Scoring explainer */}
        <div className="mt-16 glass-card rounded-2xl p-8 lg:p-12 relative overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
              <div className="inline-flex items-center gap-2 mb-4 text-gold">
                <TrendingUp className="w-4 h-4" />
                <span className="text-xs font-bold uppercase tracking-widest">{t("scoring_pretitle")}</span>
              </div>
              <h2 className="font-display text-3xl sm:text-4xl font-bold mb-1">
                {t("scoring_title")}
              </h2>
              <ul className="space-y-4">
                <ScoreItem badge="+3" title={t("scoring_3_title")} desc={t("scoring_3_desc")} variant="gold" />
                <ScoreItem badge="+1" title={t("scoring_1_title")} desc={t("scoring_1_desc")} variant="neutral" />
                <ScoreItem badge="0" title={t("scoring_0_title")} desc={t("scoring_0_desc")} variant="muted" />
              </ul>
            </div>
            <div className="relative">
              <div className="aspect-square rounded-2xl bg-gradient-to-br from-gold/20 via-transparent to-transparent border border-gold/10 flex items-center justify-center">
                <Crown className="w-48 h-48 text-gold drop-shadow-[0_0_40px_rgba(255,215,0,0.4)]" strokeWidth={1.2} />
              </div>
            </div>
          </div>
        </div>
      </section>

      <footer className="hidden md:block border-t border-white/5 py-8 text-center text-sm text-zinc-500">
        {t("footer_text")}
      </footer>
    </div>
  );
}


function ScoreItem({ badge, title, desc, variant }) {
  const cls =
    variant === "gold"
      ? "bg-gold text-black"
      : variant === "neutral"
      ? "bg-white/10 text-white"
      : "bg-white/5 text-zinc-500";
  return (
    <li className="flex items-start gap-4">
      <span className={`shrink-0 w-12 h-12 rounded-full font-black flex items-center justify-center text-lg ${cls}`}>
        {badge}
      </span>
      <div>
        <p className="font-bold text-sm">{title}</p>
        <p className="text-sm text-zinc-400">{desc}</p>
      </div>
    </li>
  );
}
